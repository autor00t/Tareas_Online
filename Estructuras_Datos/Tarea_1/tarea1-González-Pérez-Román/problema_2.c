...
void actualizarSaldos (char* clientes, char* transacciones) {
    ...
    if ((archivo_clientes = fopen(clientes, "r+")) == NULL) {
        printf("Hubo un error al abrir el archivo %s.\n", clientes);
        exit(1);
    }
    if ((archivo_transacciones = fopen(transacciones, "r")) == NULL) {
        printf("Hubo un error al abrir el archivo %s.\n", transacciones);
        exit(1);
    }
    ...
    while (fread(&cuenta, sizeof(clienteBanco), 1, archivo_clientes) != 0) {
        ...
        fseek(archivo_clientes, -1 * sizeof(clienteBanco), SEEK_CUR);
        fwrite(&cuenta, sizeof(clienteBanco), 1, archivo_clientes);
        fseek(archivo_transacciones, 0, SEEK_SET);
    }
